...
const EmeraldChittiTableNew = ({
      ...
      prevData.map((row: any) => {
        if (row.idx === id) {
          ...
          const isDuplicate = prevData.some(
            (existingRow: any) =>
              existingRow !== row &&
              Object.keys(updatedRow).every(
                (key) => key === 'idx' || updatedRow[key] === existingRow[key]
              )
          );
          if (isDuplicate) {
            toast.error('Duplicate Entry found. Please enter unique values');
            handleDeleteRow(id);
            return row;
          }
  ...
  useEffect(() => {
    if (newRowIndex !== null && inputRef.current) {
      inputRef.current.focus();
      setNewRowIndex(null); // Reset newRowIndex after focusing
    }
  }, [newRowIndex]);
