...
export const getSpecificClientGroup: any = createAsyncThunk(
  'getSpecificClientGroup/getSpecificClient',
  async (params: any) => {
    ...
    builder.addCase(getSpecificClientGroup.fulfilled, (state, action) => {
      if (
        ...
        if (action?.payload?.data?.message?.hasOwnProperty('data')) {
          state.data =
            action?.payload?.data?.message?.data?.data[0].client_group;
          state.isLoading = 'succeeded';
        }
      }
    });
    builder.addCase(getSpecificClientGroup.rejected, (state) => {
...
export const get_specific_client_group = (state: RootState) =>
